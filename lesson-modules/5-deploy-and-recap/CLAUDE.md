...
```
list ของพวกนี้ให้ดู:
1. ไฟล์ใน .claude/agents/ (ควรมี 3 ตัว: fundamentals.md + earnings.md + sentiment.md หรือชื่อใกล้เคียง)
2. .claude/skills/company-brief/SKILL.md (ควรมี + body พูดถึงการ delegate ไป sub-agents 3 ตัว)
3. ไฟล์ใน briefs/ (ขอ list ทั้งหมด นับจำนวนด้วย)
```

แล้วบอกผมว่า: (a) sub-agent files มีกี่ตัว, (b) `briefs/` มีกี่ไฟล์ และมี ticker อะไรบ้าง"

---

STOP: Wait for the student to report:
1. จำนวน sub-agent files ใน `.claude/agents/` (ต้อง = 3 หรือใกล้เคียง)
2. จำนวนและรายชื่อ ticker ใน `briefs/` folder
3. ยืนยันว่า window 2 ยังเปิดอยู่ที่ folder `my-first-project`
...
1. ถ้า sub-agents ครบ 3 + briefs มี ≥3 ไฟล์ + window 2 เปิด, say "ดี ของพร้อม showcase สมเหตุสมผล. ไปต่อ Step 1." Move to Step 1.
...
4. ถ้า sub-agent files ขาด (มีแค่ 1-2 หรือไม่มี) **และ student ไม่ได้บอกว่าข้าม L4**, ถามก่อน: "L4 ของคุณผ่านไหม sub-agent files หาย หรือคุณข้าม L4 ไปก่อน? ถ้าผ่าน ลองตรวจ `.claude/agents/` อีกที. ถ้าข้าม ของจาก L3 ก็พอ ไปต่อได้เลยบอกผมว่าจะข้าม." Wait for clarification before proceeding.
5. ถ้า window 2 ปิดไปแล้ว, ขอให้เปิด Claude Code ที่ folder `my-first-project` ใหม่ก่อนต่อ.
6. ถ้า student ใช้ folder ชื่ออื่น (ไม่ใช่ `my-first-project`), ใช้ชื่อนั้นทุกที่ที่ lesson นี้อ้าง `my-first-project`.
...
- **Services = Sub-agents** Lesson 4 คุณแยก /brief เป็น 3 specialist (fundamentals, news, sentiment) ทำงานขนาน. นี่คือ "พนักงาน" ที่ OS layer เรียกใช้
...
- 3 sub-agents เดียวกัน (fundamentals/earnings/sentiment) สแกนทั้ง list
...
4. **Lesson 4**: แตก /brief เป็น 3 sub-agent (fundamentals/10-K, earnings/transcript, sentiment/websearch) ทำงานขนาน output ผ่าน skill
